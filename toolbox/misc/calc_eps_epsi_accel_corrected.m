...
%        [P1,f1] = pwelch(data1,floor(Lscan),0,f,2*f(end));
        ...
        Pvel1  = P1.*speed ./ htotal(:,1);                                      % vel spec as function of k
        Pvel2  = P2.*speed ./ htotal(:,2);                                      % vel spec as function of k
        P1a(j,:)=P1a(j,:).*speed./ sqrt([htotal(:,1).' flipud(htotal(:,1)).' ]); 
        P2a(j,:)=P2a(j,:).*speed./ sqrt([htotal(:,2).' flipud(htotal(:,2)).' ]);
        P3a(j,:)=P3a(j,:).*speed./ sqrt([h_freq.shear.' h_freq.shear.']);
        ...
        Co(j,:)= P13.^2./(P3(j,:).*Sh1(j,:)); %transfer function between measured (mm) shear and acceleration (aa)
    end
end
       
mH1=smoothdata(abs(H1),1,'movmean',20);
mH2=smoothdata(abs(H2),1,'movmean',20);

for j=1:length(ind_cast)
    speed=Epsilon.w(j); % convert to m/s
    if speed > .2

        Sh1a(j,:) = Sh1(j,:) - abs(H1(j,:)).^2.*P3(j,:);  % Css(f) = Cmm(f) - |H(f)|^2*Caa(f)
        Sh2a(j,:) = Sh2(j,:) - abs(H2(j,:)).^2.*P3(j,:);  % Css(f) = Cmm(f) - |H(f)|^2*Caa(f)
        
        Epsilon.Pshear1{j} = Sh1a;                          % shear spec  as function of k
        Epsilon.Pshear2{j} = Sh2a;                          % shear spec  as function of k
        ...
        if tscan*speed>0.5
            [Epsilon.epsilon1(j),Epsilon.kc1(j)]=eps1_mmp(k,Epsilon.Pshear1{j},kvis,dk,Epsilon.kmax(j));
            [Epsilon.epsilon2(j),Epsilon.kc2(j)]=eps1_mmp(k,Epsilon.Pshear2{j},kvis,dk,Epsilon.kmax(j));
