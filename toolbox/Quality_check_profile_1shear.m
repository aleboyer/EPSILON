...
if question == 1
    ...
    %load('/Users/aleboyer/ARNAUD/SCRIPPS/SP1810/WW/d1/L1/Profiles_WW_ctd_d1.mat')
...
for id_profile=10:200
    try
        ...
        ax(1)=subplot('Position',[.48 .52 .5 .4]);
        ...
        minY=min(smoothdata(median(squeeze(P11(3,:,:)))));
        maxY=max(smoothdata(median(squeeze(P11(3,:,:)))));
        minY=min([minY min(smoothdata(median(squeeze(P11(5,:,:)))))]);
        maxY=max([maxY max(smoothdata(median(squeeze(P11(5,:,:)))))]);
        minY=min([minY min(smoothdata(median(squeeze(P11(6,:,:)))))]);
        maxY=max([maxY max(smoothdata(median(squeeze(P11(6,:,:)))))]);
        
        hold off
        legend('Shear1','Accel X/speed','Accel Y/speed','location','eastoutside')
        ...
        ax(2)=subplot('Position',[.48 .1 .5 .3]);
        ...
        legend('epsilon1','location','southeast')
        axis ij
        ylim(ax(4),sort(MS{id_profile}.pr([1 end])))
        xlim(ax(4),[min(MS{id_profile}.epsilon(:,1)) ...
            max(MS{id_profile}.epsilon(:,1))])
        xlabel(ax(4),'\epsilon W.kg^{-1}','fontsize',15)
        ylabel(ax(4),'Depth (m)','fontsize',15)
        grid(ax(4),'on')
        ...
        xlabel('fall rate (m s^{-1})','fontsize',15)
        ylabel('Depth (m)','fontsize',15)
        grid on
        
        Epsilon_class=calc_binned_epsi(MS(id_profile));
        
        ax(6)=subplot('Position',[.08 .1 .37 .35]);
        
        plot_binned_epsilon_1shear_sanity_profile(Epsilon_class,' ',ax(6))
